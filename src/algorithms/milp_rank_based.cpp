...
struct Model
{
    ...
    /**
     * x_jk = 1 if job j is assigned to batch k.
     * j, k in {0, ..., n-1}.
     */
    std::vector<std::vector<int>> x_jk;

    /**
     * y_k = 1 if batch k is used.
     * k in {0, ..., n-1}.
     */
    std::vector<int> y_k;

    /**
     * p_k = processing time of batch k (= max p_j for jobs j in batch k).
     * k in {0, ..., n-1}.
     */
    std::vector<int> p_k;
};
        ...
        const Instance& instance)
{
    ...
    // Variables x_jk[job_id][batch_id] = 1 if job job_id is assigned to batch batch_id.
    model.x_jk.assign(
    ...
    for (JobId job_id = 0; job_id < instance.number_of_jobs(); ++job_id) {
        for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
            model.x_jk[job_id][batch_id] = model.model.variables_lower_bounds.size();
            model.model.variables_names.push_back("x_{" + std::to_string(job_id) + "," + std::to_string(batch_id) + "}");
    ...
    // Variables y_k[batch_id] = 1 if batch batch_id is used.
    model.y_k.resize(instance.number_of_jobs());
    for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
        model.y_k[batch_id] = model.model.variables_lower_bounds.size();
        model.model.variables_names.push_back("y_" + std::to_string(batch_id));
        model.model.variables_lower_bounds.push_back(0);
        model.model.variables_upper_bounds.push_back(1);
        model.model.variables_types.push_back(mathoptsolverscmake::VariableType::Binary);
        model.model.objective_coefficients.push_back(0);
    ...
    // Variables p_k[batch_id] = processing time of batch batch_id.
    // Objective: minimize sum of p_k (equals makespan for single machine, no release dates).
    model.p_k.resize(instance.number_of_jobs());
    for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
        model.p_k[batch_id] = model.model.variables_lower_bounds.size();
        model.model.variables_names.push_back("p_" + std::to_string(batch_id));
        model.model.variables_lower_bounds.push_back(0);
    ...
    // (5) Each job is assigned to exactly one batch:
    //   sum_{batch_id} x_jk[job_id][batch_id] = 1  for all job_id.
    for (JobId job_id = 0; job_id < instance.number_of_jobs(); ++job_id) {
        model.model.constraints_starts.push_back(model.model.elements_variables.size());
        for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
            model.model.elements_variables.push_back(model.x_jk[job_id][batch_id]);
    ...
    // (6) Batch capacity:
    //   sum_{job_id} s_{job_id} * x_jk[job_id][batch_id] <= B * y_k[batch_id]  for all batch_id.
    for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
        model.model.constraints_starts.push_back(model.model.elements_variables.size());
        for (JobId job_id = 0; job_id < instance.number_of_jobs(); ++job_id) {
            model.model.elements_variables.push_back(model.x_jk[job_id][batch_id]);
            model.model.elements_coefficients.push_back(
                    static_cast<double>(instance.job(job_id).size));
        }
        model.model.elements_variables.push_back(model.y_k[batch_id]);
        model.model.elements_coefficients.push_back(
                -static_cast<double>(instance.machine(0).capacity));
    ...
    // (7) Batch processing time is the max processing time of its assigned jobs:
    //   p_k[batch_id] >= p_{job_id} * x_jk[job_id][batch_id]  for all job_id, batch_id.
    for (JobId job_id = 0; job_id < instance.number_of_jobs(); ++job_id) {
        for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
            model.model.constraints_starts.push_back(model.model.elements_variables.size());
            model.model.elements_variables.push_back(model.p_k[batch_id]);
            model.model.elements_coefficients.push_back(1.0);
            model.model.elements_variables.push_back(model.x_jk[job_id][batch_id]);
            model.model.elements_coefficients.push_back(
                    -static_cast<double>(instance.job(job_id).processing_times[0]));
    ...
    // (8) Strengthening: x_jk[job_id][batch_id] <= y_k[batch_id]  for all job_id, batch_id.
    for (JobId job_id = 0; job_id < instance.number_of_jobs(); ++job_id) {
        for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
            model.model.constraints_starts.push_back(model.model.elements_variables.size());
            model.model.elements_variables.push_back(model.x_jk[job_id][batch_id]);
        ...
        const std::vector<double>& milp_solution)
{
    ...
    // For the single machine no-release-dates case the batch order does not
    // affect the makespan, so we iterate batches by index.
    Time current_time = 0;
    for (JobId batch_id = 0; batch_id < instance.number_of_jobs(); ++batch_id) {
        if (milp_solution[model.y_k[batch_id]] < 0.5)
            continue;

        std::vector<JobId> jobs_in_batch;
        for (JobId job_id = 0; job_id < instance.number_of_jobs(); ++job_id) {
            if (milp_solution[model.x_jk[job_id][batch_id]] > 0.5)
                jobs_in_batch.push_back(job_id);
        }
        if (jobs_in_batch.empty())
            continue;

        solution_builder.append_batch(0, current_time);
        for (JobId job_id : jobs_in_batch)
            solution_builder.add_job_to_last_batch(0, job_id);

        current_time += static_cast<Time>(std::round(milp_solution[model.p_k[batch_id]]));
    }
        ...
        int*)
{
    ...
    if (!d.output.solution.feasible()
        ...
        Solution solution = retrieve_solution(d.instance, d.milp_model, milp_solution);
...
// ============================================================================
// Parallel machines, no release dates: P_m | s_j, B | C_max  (MILP_2)
// ============================================================================
...
ModelParallel create_milp_model_unrelated(
        const Instance& instance)
{
    ModelParallel model;
    model.model.objective_direction = mathoptsolverscmake::ObjectiveDirection::Minimize;
...
Solution retrieve_solution_unrelated(
        const Instance& instance,
        const ModelParallel& model,
        const std::vector<double>& milp_solution)
{
    SolutionBuilder solution_builder;
    solution_builder.set_instance(instance);
...
class EventHandlerParallel: public CbcEventHandler
{
...
public:
    ...
    EventHandlerParallel(
            const Instance& instance,
            const MilpRankBasedParameters& parameters,
            const ModelParallel& milp_model,
            Output& output,
            AlgorithmFormatter& algorithm_formatter):
        CbcEventHandler(),
        instance_(instance),
    ...
    virtual ~EventHandlerParallel() { }

    EventHandlerParallel(const EventHandlerParallel& rhs):
        CbcEventHandler(rhs),
        instance_(rhs.instance_),
        parameters_(rhs.parameters_),
        milp_model_(rhs.milp_model_),
    ...
    virtual CbcEventHandler* clone() const { return new EventHandlerParallel(*this); }

private:

    const Instance& instance_;
    const MilpRankBasedParameters& parameters_;
    const ModelParallel& milp_model_;
    Output& output_;
    AlgorithmFormatter& algorithm_formatter_;

};

CbcEventHandler::CbcAction EventHandlerParallel::event(CbcEvent which_event)
{
    // Not in subtree.
    if ((model_->specialOptions() & 2048) != 0)
        return noAction;
    ...
    if (!output_.solution.feasible()
            || output_.solution.makespan() > milp_objective_value) {
        std::vector<double> milp_solution = mathoptsolverscmake::get_solution(cbc_model);
        Solution solution = retrieve_solution_unrelated(instance_, milp_model_, milp_solution);
        algorithm_formatter_.update_solution(solution, "node " + std::to_string(number_of_nodes));
    }
...
struct XpressCallbackUserParallel
{
    const Instance& instance;
    const MilpRankBasedParameters& parameters;
    const ModelParallel& milp_model;
...
void xpress_callback_unrelated(
        XPRSprob xpress_model,
        void* user,
        int*)
{
    const XpressCallbackUserParallel& d = *(const XpressCallbackUserParallel*)(user);
    ...
    if (!d.output.solution.feasible()
            || d.output.solution.makespan() > milp_objective_value) {
        std::vector<double> milp_solution = mathoptsolverscmake::get_solution(xpress_model);
        Solution solution = retrieve_solution_unrelated(d.instance, d.milp_model, milp_solution);
        d.algorithm_formatter.update_solution(solution, "");
    }
...
Output batchschedulingsolver::milp_rank_based(
        const Instance& instance,
        const MilpRankBasedParameters& parameters)
{
    Output output(instance);
    AlgorithmFormatter algorithm_formatter(instance, parameters, output);
    algorithm_formatter.start("Rank-based MILP");
    ...
    if (parameters.solver == mathoptsolverscmake::SolverName::Cbc) {
        ...
        XpressCallbackUser xpress_callback_user{instance, parameters, milp_model, output, algorithm_formatter};
...
Output batchschedulingsolver::milp_rank_based_release_dates(
        const Instance& instance,
        const MilpRankBasedParameters& parameters)
{
    throw std::invalid_argument("milp_rank_based_release_dates: not implemented");
    return Output(instance);
}

Output batchschedulingsolver::milp_rank_based_unrelated(
        const Instance& instance,
        const MilpRankBasedParameters& parameters)
{
    Output output(instance);
    AlgorithmFormatter algorithm_formatter(instance, parameters, output);
    algorithm_formatter.start("Rank-based MILP (parallel)");

    algorithm_formatter.print_header();

    ModelParallel milp_model = create_milp_model_unrelated(instance);
    ...
    if (parameters.solver == mathoptsolverscmake::SolverName::Cbc) {
        ...
        mathoptsolverscmake::set_time_limit(cbc_model, parameters.timer.remaining_time());
        mathoptsolverscmake::load(cbc_model, milp_model.model);
        EventHandlerParallel cbc_event_handler(instance, parameters, milp_model, output, algorithm_formatter);
        cbc_model.passInEventHandler(&cbc_event_handler);
        mathoptsolverscmake::solve(cbc_model);
        milp_solution = mathoptsolverscmake::get_solution(cbc_model);
        milp_bound = mathoptsolverscmake::get_bound(cbc_model);
        ...
        highs.setCallback([
                    ...
                    void*)
                {
                    if (!highs_output->mip_solution.empty()) {
                        ...
                        if (!output.solution.feasible()
                                || output.solution.makespan() > milp_objective_value) {
                            Solution solution = retrieve_solution_unrelated(instance, milp_model, highs_output->mip_solution);
                            algorithm_formatter.update_solution(solution, "node " + std::to_string(highs_output->mip_node_count));
                        }
        ...
        XpressCallbackUserParallel xpress_callback_user{instance, parameters, milp_model, output, algorithm_formatter};
        XPRSaddcbprenode(xpress_model, xpress_callback_unrelated, (void*)&xpress_callback_user, 0);
        mathoptsolverscmake::solve(xpress_model);
        milp_solution = mathoptsolverscmake::get_solution(xpress_model);
        milp_bound = mathoptsolverscmake::get_bound(xpress_model);
        XPRSdestroyprob(xpress_model);
    ...
    // Retrieve solution.
    Solution solution = retrieve_solution_unrelated(instance, milp_model, milp_solution);
    algorithm_formatter.update_solution(solution, "");
...
Output batchschedulingsolver::milp_rank_based_unrelated_release_dates(
        const Instance& instance,
        const MilpRankBasedParameters& parameters)
{
    throw std::invalid_argument("milp_rank_based_unrelated_release_dates: not implemented");
    return Output(instance);
}
